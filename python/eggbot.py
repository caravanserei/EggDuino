...
# Version 2.7.1a, dated March 11, 2016 by AS.
#
...
class EggBot( inkex.Effect ):
	def __init__( self ):
		...
		#AS	
		self.OptionParser.add_option( "--invertServo",
			action="store", type="inkbool",
			dest="invertServo", default=False,
			help="Invert motion of servo within 180deg." )
 		...
 		if ((self.options.tab == '"Help"') or (self.options.tab == '"options"')  or (self.options.tab == '"timing"')):
			...
			if self.options.tab == '"splash"':
				...
				if ( self.LayersPlotted == 0 ):
					inkex.errormsg( gettext.gettext( "I did not find any numbered layers to plot." ) )
...
# 						inkex.errormsg('lastpathnc: ' + str(self.svgLastPathNC))
	...
	def manualCommand( self ):
		...
		if self.options.manualType == "raise-pen":
			...
			inkex.errormsg( 'EBB replied with version:\n ' + strVersion )
	...
	def engraverOff( self ):
		...
		if self.options.engraving:
			ebb_serial.command( self.serialPort, 'PO,B,3,0\r')	#Eggbot	
			self.engraverIsOn = False
